...
export default function ReviewPostScreen(props) {
    ...
    const submitHandler = (e) => {
        ...
        if (rating === 0) {
            ...
            dispatch(post(clubId, rating, timeCommitment, recommend, classField, filter.clean(major), filter.clean(position), filter.clean(comment))).then(reviewData => {
                return dispatch(detailsClub(clubId)).then(clubData => {
            ...
            dispatch(post(clubId, rating, timeCommitment, recommend, classField, filter.clean(major), filter.clean(position), filter.clean(comment))).then(reviewData => {
                return dispatch(postInterview(clubId, difficulty, filter.clean(major), filter.clean(roleApplied), filter.clean(interviewComment))).then(interviewData => {
                    return dispatch(detailsClub(clubId)).then(clubData => {
                        let path = `/club/${schoolId}/${clubId}`;
                        history.push(path);
                    })
    ...
    return (
        <div className="grid-container">
            ...
            <main>
                <div>
                    {(loadingClub) ? <LoadingBox></LoadingBox> :
                        (errorClub) ? <MessageBox variant="danger">{errorClub}</MessageBox> :
                            windowWidth > 930 ?
                                (
                                    <div>
                                        <form className="form" onSubmit={submitHandler} onKeyDown={(e) => checkKeyDown(e)}>
                                            <div>
                                                <h1 style={{ marginTop: "-1rem", marginBottom: "-1rem" }}>Rate {club.name}</h1>
                                            </div>
                                            <div>
                                    ...
                                    <div style={{ marginTop: "4rem" }}>
                                        <form className="form" onSubmit={submitHandler}>
                                            <div className="form-wrapper">
                                                <div>
                                                    <h1 style={{ marginTop: "-1rem", marginBottom: "-1rem", textAlign: "center" }}>Rate {club.name}</h1>
                                                </div>
                                                <div>
